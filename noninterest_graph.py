...
min_year = 1940
max_year = nonint_df['Year'].max()
min_nonint = nonint_df['spend_nonint_gdp'].min()
max_nonint = nonint_df['spend_nonint_gdp'].max()

#Finding the index for when minimum year the graph will show
start_year_index = -1
for x in range(0,data_length):
    if(nonint_df['Year'][x]==min_year):
        start_year_index = x
        break

# Output to HTML file titled: "federal_debt_image.html"
fig_title = ('U.S. Federal Noninterest Outlays as Percent of Gross Domestic '+ 
                'Product by Party Control: 1940-2020')
fig_path = os.path.join(images_dir, 'noninterest_image.html')
...
fig = figure(title=fig_title,
             ...
             y_range=(min_nonint - 3, max_nonint + 3),
...
for x in range(start_year_index, recession_data_length):
      ...
      if(peak_year >= min_year and trough_year >= min_year):
            fig.patch(  x=[peak_year, trough_year, trough_year,peak_year],
                        y=[0, 0, max_nonint + 10, max_nonint + 10],
                        fill_color='gray',
                        fill_alpha=0.4,
                        line_width=0,
...
# Plotting the line
fig.line(x='Year', y='spend_nonint_gdp', source=nonint_cds, color='#423D3C',line_width=2,name='line')

# Plotting the dots representing party control
for x in range(start_year_index, data_length):
      if(nonint_df["RepHouseSeats"][x] > 217 and
         nonint_df["DemWhitehouse"][x] == 0):
            fig.circle(x=nonint_df["Year"][x],
      ...
      elif (nonint_df["DemHouseSeats"][x] > 217 and
...
fig.add_tools(HoverTool(tooltips=tooltips,names=['line']))
