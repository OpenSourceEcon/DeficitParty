...
fig_title = 'Federal Receipts as Percent of Gross Domestic Product by Party Control: 1929-2020'
...
for x in range(0,recession_data_length):
      ...
      if(peak_year == trough_year and peak_year >= min_year and trough_year >= min_year):
            fig.patch(  x=[peak_year,trough_year+1,trough_year+1,peak_year], 
                        y=[0,0,receipts_max+10,receipts_max+10],
                        fill_color='gray',
                        fill_alpha=0.4,
                        line_width=0,
...
#Plotting the line
fig.line(   x='Year', y='receipts_gdp', 
            source=revenue_cds, 
            color='#423D3C', 
            line_width=2,
            legend_label = 'Revenues/GDP')

#Plotting the dots representing party control
for x in range(0,data_length):
      if(revenue_df["RepHouseSeats"][x]>217 and revenue_df["DemWhitehouse"][x]==0):
            fig.circle(x=revenue_df["Year"][x], y=revenue_df["receipts_gdp"][x],
                         size=10,
                         fill_color='red',
                         legend_label = 'Republican Control')
      elif (revenue_df["DemHouseSeats"][x]>217 and revenue_df["DemWhitehouse"][x]==1):
            fig.circle(x=revenue_df["Year"][x], y=revenue_df["receipts_gdp"][x],
                         size=10,
                         fill_color='blue',
...
hover_glyph = fig.circle(x='Year', y='receipts_gdp', source=revenue_cds,size=10, alpha=0,
