...
min_seats = receipts_df['DemHouseSeats'].min()
max_seats = receipts_df['DemHouseSeats'].max()

# Output to HTML file
fig_title = ('U.S. Federal Receipts as Percent of Gross Domestic Product by '+
              'Democrat House Seats: 1929-2020')
fig_path = os.path.join(images_dir, 'receipts_house_plot.html')
...
fig = figure(title=fig_title,
             ...
             y_range=(min_receipts - 4, max_receipts + 4),
...
for x in range(0,data_length):
      if(receipts_df["RepHouseSeats"][x] > 217 and
         receipts_df["DemWhitehouse"][x] == 0):
            fig.circle(x=receipts_df["DemHouseSeats"][x],
      ...
      elif (receipts_df["DemHouseSeats"][x] > 217 and
...
fig.scatter(  x='DemHouseSeats', y='receipts_gdp', source=receipts_cds, 
